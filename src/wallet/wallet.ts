...
export const wallet = {
    setupWebKeplr: async (config: Config):
    ...
    setupCosmostation: async (config: Config):
        Promise<SigningCosmWasmClient> => {
        // Setup signer
    ...
    setupNodeLocal: async (config: Config, mnemonic: string, WalletOptions: any):
    ...
    setupWebLedger: async (config: Config, transport: any, WalletOptions: any):
        Promise<SigningCosmWasmClient> => {
